...
PROD_SERVER_SSH := dev-vds
STU_SERVER_SSH := dev-vps
BASE_DIR = fastapi-template
...
deploy-configs:  ## Deploy configs
	scp ./.env.prod ./Makefile ./docker-compose.yaml ./postgresql.conf $(PROD_SERVER_SSH):/$(BASE_DIR)/$(PROJECT_NAME)
...
deploy-configs-stu:  ## Deploy configs on stu
	scp ./Makefile ./docker-compose.test.yaml ./postgresql.conf $(STU_SERVER_SSH):/$(BASE_DIR)/$(PROJECT_NAME)
...
docker-buildx-prod: ## Build (buildx multi-platform) prod image
	docker buildx build --platform linux/amd64,linux/arm64 -t $(DOCKER_USERNAME)/$(PROJECT_NAME):latest -t $(DOCKER_USERNAME)/$(PROJECT_NAME):$(PROJECT_VERSION) .

docker-buildx-test: ## Build (buildx multi-platform) test image
	docker buildx build --platform linux/amd64,linux/arm64 -t $(DOCKER_USERNAME)/$(PROJECT_NAME):test -t $(DOCKER_USERNAME)/$(PROJECT_NAME):$(PROJECT_VERSION)-test .

docker-buildx-and-push-prod: ## Build (buildx) and push prod image
	docker buildx build --platform linux/amd64,linux/arm64 -t $(DOCKER_USERNAME)/$(PROJECT_NAME):latest -t $(DOCKER_USERNAME)/$(PROJECT_NAME):$(PROJECT_VERSION) --push .

docker-buildx-and-push-test: ## Build (buildx) and push test image
	docker buildx build --platform linux/amd64,linux/arm64 -t $(DOCKER_USERNAME)/$(PROJECT_NAME):test -t $(DOCKER_USERNAME)/$(PROJECT_NAME):$(PROJECT_VERSION)-test --push .
